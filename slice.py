...
def compute_slice_metrics(model, encoder, lb, cleaned_df,
                          categorical_features, slice_features):
    """
    ...
    for value in cleaned_df[slice_features].unique():
        ...
        X_slice, y_slice, _, _ = process_data(
            X_slice, categorical_features, label="salary",
            training=False,
            encoder=encoder,
            lb=lb)
        preds = inference(model, X_slice)
        print(
            f"shape of preds: {preds.shape}  \
                & shape of y_slice: {y_slice.shape}")
        precision, recall, fbeta = compute_model_metrics(y_slice, preds)
        slice_metrics[value] = {'Precision': precision,
                                'Recall': recall, 'Fbeta': fbeta}
        print(
            f"Slice metrics for {slice_features} = \
            {value}: {slice_metrics[value]}")
